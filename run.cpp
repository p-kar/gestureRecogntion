...
char* getPredictionMATLAB(Engine* ep, deque<double> &Ax, deque<double> &Bx, deque<double> &Cx, deque<double> &Dx, deque<double> &Ex, deque<double> &Ay, deque<double> &By, deque<double> &Cy, deque<double> &Dy, deque<double> &Ey)
{
	if(Ax.size() < 30)
	{
		char* pred = (char*) malloc(10);
		strcpy(pred, "none");
		return pred;
	}
	...
	engEvalString(ep, "pred = getPrediction(T);");
	...
	while(1)
	{
	    ...
	    if(record)
	    {
			...
			if(Ey.size() > time_steps)
				Ey.pop_front();
			char* pred = getPredictionMATLAB(ep, Ax, Bx, Cx, Dx, Ex, Ay, By, Cy, Dy, Ey);
			// printf("Predicted gesture: %s\n", pred);
			char arr[100];
			sprintf(arr, "Predicted Gesture: %s", pred);
			CvFont font;
			cvInitFont(&font, CV_FONT_HERSHEY_SIMPLEX, 0.5, 0.5);
			cvPutText(frame, arr, cvPoint(10, 15), &font, CV_RGB(255, 0, 0));
	    }
